...
<div class="hidden lg:block bg-primary text-white/70 text-xs py-2">
...
<header id="header" class="fixed top-0 lg:top-8 left-0 right-0 z-50 transition-all duration-500 [&.scrolled]:bg-white/95 [&.scrolled]:backdrop-blur-xl [&.scrolled]:shadow-lg [&.scrolled]:lg:top-0 {{ $isHome ? 'bg-transparent' : 'bg-white shadow-sm lg:top-0' }}">
        ...
        <div class="flex items-center justify-between h-16 lg:h-20">
            <a href="{{ route('accueil') }}" class="flex items-center gap-2">
                ...
                @else
                    ...
                    <div>
                        <span class="text-lg font-bold {{ $isHome ? 'text-white [header.scrolled_&]:text-primary' : 'text-primary' }}">ADENIKE</span>
                        <span class="text-lg font-bold text-accent">-INTER</span>
                    </div>
                @endif
            </a>
                ...
                @foreach([
                    ...
                    <a href="{{ route($link['route']) }}"
                       ...
                       {{ request()->routeIs($link['route'])
                           ...
                           : ($isHome
                               ? 'text-white/80 hover:text-white hover:bg-white/10 [header.scrolled_&]:text-gray-600 [header.scrolled_&]:hover:text-accent'
   ...
   class="ml-4 px-6 py-2.5 bg-accent hover:bg-accent-dark text-white rounded-xl text-sm font-semibold transition-all hover:shadow-lg hover:shadow-accent/30 hover:-translate-y-0.5 flex items-center gap-2">

    <!-- Icône tête avec casque -->
    <span class="flex items-center gap-1.5">
    ...
    <span>Support client</span>
            ...
            <button id="menuToggle" class="lg:hidden p-2 rounded-lg {{ $isHome ? 'text-white [header.scrolled_&]:text-primary' : 'text-primary' }}">
    ...
    <div id="mobileMenu" class="lg:hidden bg-white border-t border-gray-100 shadow-xl hidden">
        <div class="px-6 py-4 space-y-1">
            @foreach(['accueil' => 'Accueil', 'apropos' => 'À propos', 'produits' => 'Produits', 'realisations' => 'Réalisations', 'contact' => 'Contact'] as $r => $l)
                <a href="{{ route($r) }}" class="block px-4 py-3 rounded-xl {{ request()->routeIs($r) ? 'bg-accent/10 text-accent' : 'text-gray-700 hover:bg-accent/5 hover:text-accent' }} font-medium">{{ $l }}</a>
            @endforeach
        </div>
    </div>
</header>